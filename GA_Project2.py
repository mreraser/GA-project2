...
if __name__ == "__main__":
    ...
    user_input = raw_input("원하는 문장(염색체)을 입력하세요: ")
    print("입력한 문장 "+ user_input +"의 각 문자는 유전자가 됩니다.")
    ...
    class GuessText(GeneticFunctions):
        ...
        def check_stop(self, fits_populations):
            ...
            if self.counter % 5 == 0:
                ...
                print(
                    "[세대 %3d] 적합도=(최대 : %4d, 평균: %4d, 최소: %4d): 염색체: %r" %
        ...
        def chromo2text(self, chromo):
            return "".join(chr(max(1, min(ch, 128))) for ch in chromo)

        def random_chromo(self):
            return [random.randint(1, 128) for i in range(len(self.target))]
        pass

    #user_input = input("원하는 문장(염색체)을 입력하세요: ")
    #print("입력한 문장 "+ user_input +"의 각 문자는 유전자가 됩니다.")
    GeneticAlgorithm(GuessText(user_input)).run()
    pass
